...
${member.workDescription ? `<p class="text-sm text-leaf font-medium mt-1">${member.workDescription}</p>` : ''}
        ...
        // Others: Ultra-compact single line layout
        grid.className = 'space-y-0';
        ...
        members.forEach((member) => {
          ...
          card.innerHTML = `
                <div class="flex items-center gap-2">
                    ...
                    ${member.workDescription ? `<span class="text-gray-400">·</span><span class="text-xs text-gray-500 dark:text-gray-400">${member.workDescription}</span>` : ''}
                </div>
            `;
          grid.appendChild(card);
        });
