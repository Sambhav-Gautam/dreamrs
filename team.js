...
${member.workDescription ? `<p class="text-sm text-leaf font-medium mt-1">${member.workDescription}</p>` : ''}
                  </div>
        ...
        members.forEach((member) => {
          ...
          card.innerHTML = `
                <div class="grid grid-cols-[auto_60px_1fr] items-center gap-3">
                    <span class="font-medium text-gray-800 dark:text-white whitespace-nowrap">${member.name}</span>
                    <div class="flex items-center gap-2">
                    ...
                    ${member.workDescription ? `<span class="text-xs text-gray-500 dark:text-gray-400 truncate">${member.workDescription}</span>` : '<span></span>'}
                </div>
            `;
          grid.appendChild(card);
        });
